...
type BackupScheduleLister interface {
	...
	List(selector labels.Selector) (ret []*v1beta1.BackupSchedule, err error)
...
// List lists all BackupSchedules in the indexer.
func (s *backupScheduleLister) List(selector labels.Selector) (ret []*v1beta1.BackupSchedule, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1beta1.BackupSchedule))
	})
	return ret, err
}
...
type BackupScheduleNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1beta1.BackupSchedule, error)
...
// List lists all BackupSchedules in the indexer for a given namespace.
func (s backupScheduleNamespaceLister) List(selector labels.Selector) (ret []*v1beta1.BackupSchedule, err error) {
...
// Get retrieves the BackupSchedule from the indexer for a given namespace and name.
func (s backupScheduleNamespaceLister) Get(name string) (*v1beta1.BackupSchedule, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1beta1.Resource("backupschedule"), name)
	}
	return obj.(*v1beta1.BackupSchedule), nil
}
