...
KAHUMGR_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd -P)"
source "${KAHUMGR_ROOT}/build/lib/logging.sh"

readonly KAHU_GOPATH="${KAHUMGR_ROOT}/${OUT_DIR}/go"
readonly KAHU_GO_PACKAGE=github.com/soda-cdm/kahu-manager
...
golang::create_gopath_tree() {
  ...
  if [[ ! -e "${go_pkg_dir}" || "$(readlink "${go_pkg_dir}")" != "${KAHUMGR_ROOT}" ]]; then
    ln -snf "${KAHUMGR_ROOT}" "${go_pkg_dir}"
  fi
}
...
golang::build_binaries() {
  ...
  (
    ...
    goasmflags="-trimpath=${KAHUMGR_ROOT}"
    gogcflags="${GOGCFLAGS:-} -trimpath=${KAHUMGR_ROOT}"
